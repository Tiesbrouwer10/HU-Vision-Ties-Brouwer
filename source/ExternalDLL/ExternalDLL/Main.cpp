...
int test(int argc, char* argv[]) {
  ...
  std::string picturelocation =
      "C:\\Users\\ties1\\OneDrive\\Bureaublad\\STUDIE\\HBO-ICT jaar "
      "2019-2020\\Vision\\Github repo\\HU-Vision-Ties-Mick\\testsets\\Set C "
      "Lightness\\faceFolder1\\";
  std::ofstream debugoutput(picturelocation + "Intensity.csv");
  ImageIO::debugFolder = picturelocation + "\\debugFolder";
  ImageIO::isInDebugMode = true;  // If set to false the ImageIO class will skip
                                  // any image save function calls
  ...
  for (int testPicture = 1; testPicture <= 100; testPicture++) {
    pictureHitrate = 0;
